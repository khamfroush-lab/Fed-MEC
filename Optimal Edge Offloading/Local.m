function R = Local(Requests, Edges, Edges_Models, Max_Delay, Max_Accuracy, Communication_Delays, AverageQueueDelay, w1, w2)
...
%iteration = 1;
%counter = 0;
Results = zeros(n, 5); %,feasible, edge number, edge model, accuracy, delay
%Fitness_req = zeros(n,2); % req number, fitness_get               %edge number, fitness, acc, delay
    %%choosing based on least Communication delay and accuracy
for i = 1: length(Requests(:,1))
    source_req = Requests(i,2);
    Fitness_edge = zeros(length(find(Edges_Models(:,1)==source_req)),4);
    %j = 0;
    counter_fitness = 0;
    for j = 1: length(Edges_Models)
        if source_req == Edges_Models(j,1) && Edges_Models(j,3) == Requests(i,1)...
            && Edges_Models(j,5) >= Requests(i,3) ...
                && ((Edges_Models(j,6)+ AverageQueueDelay(i) + Requests(i,5) ...
...
%                 Requests(i,5)))/Max_Delay));
                ...
                Requests(i,5)))/Max_Delay);
%             j = j + 1;
%             Fitness_edge(j,1) = Edges_Models(u,1);
%             Fitness_edge(j,2) = (w1(i,1)*((Edges_Models(u,5)- Requests(i,3))/Max_Accuracy )) +...
%             (w2(i,1)*(( Requests(i,4)- (Edges_Models(u,6)+ Requests(i,5) +...
%             (Communication_Delays(source_req,Edges_Models(u,1)) * 2) + AverageQueueDelay(i) + ...
%         Requests(i,5)))/Max_Delay));
%             Fitness_edge(j,3) = Edges_Models(u,5);
%             Fitness_edge(j,4) = Edges_Models(u,6);
        end
    end
    Fitness_edge = Fitness_edge(Fitness_edge(:,1)>0,:);
    %Fitness_edge = Fitness_edge(randperm(size(Fitness_edge, 1)), :);
    Fitness_edge = sortrows(Fitness_edge,2,'descend');
    for j=1 : length(Fitness_edge(:,1))
        if Edges(Fitness_edge(j,1),3) >= 1 && Fitness_edge(j,1) == source_req 
            %...& Fitness_edge(j,3) >= Requests(i,3) ...
