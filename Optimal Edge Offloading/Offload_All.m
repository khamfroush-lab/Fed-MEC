function R = Offload_All(Requests, Edges, Edges_Models, Max_Delay, Max_Accuracy, Communication_Delays, AverageQueueDelay, w1, w2)
...
number_of_servers = length(find(Edges_Models(:,2)==0));
Fitness_edge = zeros(number_of_servers,4);               %edge number, fitness, acc, delay
while counter < iteration
    ...
    for i = 1: length(Requests(:,1))
        ...
        for j = 1: length(Edges_Models(:,1)) 
            if Edges_Models(j,3) == Requests(i,1) && Edges_Models(j,2) == 0 
