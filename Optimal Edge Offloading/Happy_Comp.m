function R = Happy_Comp(Requests, Edges, Edges_Models, Max_Delay, Max_Accuracy, Communication_Delays, AverageQueueDelay, w1, w2)
...
%                 Requests(i,5)))/Max_Delay));
        ...
        for j=1 : length(Fitness_edge(:,1))
            if (Fitness_edge(j,1) ~= source_req) && Edges(source_req,2) >= 1 && Edges(Fitness_edge(j,1),3) >= 1
