function R = Random(Requests, Edges, Edges_Models, Max_Delay, Max_Accuracy, Communication_Delays, AverageQueueDelay, w1, w2)
...
Fitness_edge = zeros(length(Edges_Models),4);               %edge number, fitness, acc, delay
while counter < iteration
    ...
    for i = 1: length(Requests(:,1))
        ...
        for j = 1: length(Fitness_edge(:,1))
           if Edges_Models(j,3) == Requests(i,1)&& Edges_Models(j,3) == Requests(i,1)...
...
%                     (w2(i,1)*(( Requests(i,4)- (Edges_Models(j,6)+ Requests(i,5) +...
%                     (Communication_Delays(source_req,Edges_Models(j,1)) * 2) + AverageQueueDelay(i) + ...
%                 Requests(i,5)))/Max_Delay));
                if alfa >= 0 && beta >= 0
                    Fitness_edge(counter_fitness,2) = 1/ (1 + exp(-(alfa + beta)));
